...
type AuthResponse struct {
	...
	UserID   int64        `json:"account_id,omitempty"`
...
func (c *Client) Login(account, password string) (*AuthResponse, error) {
	...
	if authResp.Success {
		...
		if len(authResp.Servers) > 0 {
			...
			c.gatewayAddr = c.selectedServer.GetAddr()
		}
	}
...
func main() {
	...
	case "1":
		...
		resp, err := client.Login(account, password)
		if err != nil {
			fmt.Printf("登录失败: %v\n", err)
			os.Exit(1)
		}
	...
	case "2":
		...
		resp, err := client.Register(account, password, email)
		if err != nil {
			fmt.Printf("注册失败: %v\n", err)
			os.Exit(1)
		}
	...
	if !serverListResp.Success {
		fmt.Printf("获取服务器列表失败: %s\n", serverListResp.ErrorMsg)
		os.Exit(1)
	}
