...
> **本文与实现现状的关系（成熟化改造后校准，2026-07-24）**：本文是**架构设计文档**（高层设计意图），
> 与当前代码在整体架构、四服职责、地图模式、通信链路上**broadly 一致**。但对"已实现且已验证"的**权威现状**，
> 以下文档才是真相源（本文不重复其细节，避免多处漂移）：
> - **跨服协议的确切契约**（线格式 / protoId / 编解码唯一源）→ [`协议契约.md`](协议契约.md)（本文 §6.2 只给高层描述）。
> - **已实现并有测试/E2E 背书的能力** → [`成熟化改造-执行计划.md`](成熟化改造-执行计划.md) 台账 + [`项目评估报告.md`](项目评估报告.md) 能力→证据清单。
> - **关键决策与领域逻辑分层**（zCommon/game 领域模型层 vs MapServer 战斗运行时层）→ [`框架成熟化改造方案.md`](框架成熟化改造方案.md) 三.1。
>
> **成熟化已落地、可视为对本设计的具体实现/强化**：Game↔Map 统一 `net/service`+全 protobuf（旧反连 `connection` 死代码已删）、
> Outbox/Inbox 可持久化（§8 一致性设计的落地，MySQL 验证）、连接去重 + zNet 断线自动重连硬化（§5.4 的连接级实现）、
> Actor 监督（panic 恢复）、zMetrics 接入 zNet。**本文 §5.4 的"客户端会话态恢复"仍为设计愿景**（连接级重连+在途消息连续性已实现，完整登录态跨进程恢复待续）。
