...
func (m *Metrics) Start() error {
	...
	go func() {
		...
		if err != nil {
			zLog.Error("Failed to start metrics server", zap.Error(err))
		}
	}()
...
func (m *Metrics) IncrementConnections() {
	if counter := m.metricsMgr.GetCounter("gateway_connections_total"); counter != nil {
		counter.Inc()
	}
	if gauge := m.metricsMgr.GetGauge("gateway_connections_current"); gauge != nil {
...
func (m *Metrics) IncrementMessagesReceived() {
	if counter := m.metricsMgr.GetCounter("gateway_messages_received_total"); counter != nil {
		counter.Inc()
	}
}
...
func (m *Metrics) IncrementMessagesSent() {
	if counter := m.metricsMgr.GetCounter("gateway_messages_sent_total"); counter != nil {
		counter.Inc()
	}
}
...
func (m *Metrics) IncrementTokenVerifications() {
	if counter := m.metricsMgr.GetCounter("gateway_token_verifications_total"); counter != nil {
		counter.Inc()
	}
}
...
func (m *Metrics) IncrementTokenFailures() {
	if counter := m.metricsMgr.GetCounter("gateway_token_failures_total"); counter != nil {
		counter.Inc()
	}
}
